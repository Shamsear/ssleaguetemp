...
try {
  ...
  files.forEach(file => {
    if (file.includes('inspect_') || file.includes('search_') || file.includes('find_')) return;
    const content = fs.readFileSync(file, 'utf8');
    if (content.includes('>= 16') && (content.includes('player_seasons') || content.includes('realplayerstats'))) {
