...
async function processTeam(teamSeasonDoc, seasonId) {
  ...
  // Get fixture for round 4 from Neon
  ...
  if (fixtures.length === 0) {
    console.log(`   ⚠️  No fixture found for round 4 - SKIPPING`);
  ...
  const hasExisting = existingTxns.docs.some(doc => {
    ...
    return metadata.fixture_id === fixtureId && metadata.round_number === 4;
  });
  
  if (hasExisting) {
    console.log(`   ⚠️  Transactions already exist for round 4 - SKIPPING`);
...
async function createAllTeamsSalaryTransactions() {
  try {
    ...
    console.log('🎯 Processing ROUND 4 ONLY\n');
