/**
 * Create salary deduction transactions for Los Galacticos
 * Rounds 1, 2, 3, 5 (skipping round 4)
 * 
...
async function createSalaryTransactions() {
  try {
    ...
    // Find Los Galacticos team_season
    ...
    // Get fixtures for rounds 1, 2, 3, 5 from Neon (skipping round 4)
    console.log('\n🔍 Finding fixtures for rounds 1, 2, 3, 5...');
    
    const fixtures = await sql`
      SELECT id, round_number, home_team_id, away_team_id, status, home_score, away_score, season_id
      FROM fixtures
      WHERE season_id = ${seasonId}
      AND round_number IN (1, 2, 3, 5)
      AND (home_team_id = ${teamId} OR away_team_id = ${teamId})
      ORDER BY round_number ASC
    `;
    
    console.log(`   Found ${fixtures.length} fixtures for Los Galacticos in rounds 1, 2, 3, 5`);
    
    if (fixtures.length === 0) {
      console.error('❌ No fixtures found for Los Galacticos in rounds 1, 2, 3, 5');
      return;
    }
    ...
    for (const fixture of fixtures) {
      ...
      matchups.forEach(matchup => {
        // Determine if Los Galacticos was home or away
      ...
      console.log(`   Los Galacticos players: ${playerIds.size}`);
