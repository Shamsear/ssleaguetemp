...
async function recalculateFantasyPlayerPoints() {
  ...
  try {
      ...
      for (const f of fixtures) {
        ...
        for (const s of sides) {
          ...
          for (const ft of lTeams) {
            ...
            if (currentSupported && currentSupported.startsWith(s.tid + '_')) {
              totals.get(ft.team_id).passive += val;
              bonusRecords.push({ league_id: lid, team_id: ft.team_id, real_team_id: s.tid, real_team_name: ft.supported_team_name, fixture_id: f.id, round_number: f.round_number, total_bonus: val, calculated_at: new Date() });
            }
          }
        }
      }
    ...
    console.log('Updating fantasy_squad...');
    for (const s of squad) {
    ...
    console.log(`Updated ${squad.length} squad members`);
    
    console.log('\nUpdating fantasy_teams...');
    for (const t of teams) {
      // Get team bonuses (TOD/TOW awards)
      const teamBonus = adminBonuses.filter(b => b.target_type === 'team' && b.target_id === t.supported_team_id).reduce((sum, b) => sum + Number(b.points), 0);
      
      // Get player bonuses (POTD/POTW awards) for players in this team's squad
      const teamPlayerIds = squad.filter(s => s.team_id === t.team_id).map(s => s.real_player_id);
      const playerBonus = adminBonuses.filter(b => b.target_type === 'player' && teamPlayerIds.includes(b.target_id)).reduce((sum, b) => sum + Number(b.points), 0);
      
      const playerPts = totals.get(t.team_id).player;
      const passivePts = totals.get(t.team_id).passive + teamBonus + playerBonus;
      const totalPts = playerPts + passivePts;
      console.log(`  ${t.team_name}: Player=${playerPts}, Passive=${passivePts} (Base=${totals.get(t.team_id).passive}, TeamBonus=${teamBonus}, PlayerBonus=${playerBonus}), Total=${totalPts}`);
      await fantasyDb`UPDATE fantasy_teams SET player_points=${playerPts}, passive_points=${passivePts}, total_points=${totalPts}, updated_at=NOW() WHERE team_id=${t.team_id}`;
    }
