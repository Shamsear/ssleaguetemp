/**
 * Create salary deduction transactions for La Masia
 * Rounds 1-4
 * 
...
async function createSalaryTransactions() {
  try {
    console.log('🔍 Finding La Masia team...');
    
    // Find La Masia team_season
    const teamSeasonsSnapshot = await db.collection('team_seasons')
      .where('team_name', '==', 'La Masia')
      .where('status', '==', 'registered')
      .get();
    
    if (teamSeasonsSnapshot.empty) {
      console.error('❌ La Masia team_season not found');
      return;
    }
    ...
    console.log(`   Found ${fixtures.length} fixtures for La Masia in rounds 1-4`);
    
    if (fixtures.length === 0) {
      console.error('❌ No fixtures found for La Masia in rounds 1-4');
      return;
    }
    ...
    for (const fixture of fixtures) {
      ...
      matchups.forEach(matchup => {
        // Determine if La Masia was home or away
      ...
      console.log(`   La Masia players: ${playerIds.size}`);
