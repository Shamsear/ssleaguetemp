/**
 * PHASE 4B: Fix TeamStats ID to Include Season
 * 
 * This script fixes the teamstats ID to include season_id to handle cases
 * where multiple seasons use the same tournament_id (e.g., 'historical')
 * 
 * Changes:
 * 1. Changes id from teamid_tournamentid to teamid_seasonid_tournamentid
 * 2. Migrates existing data to new format
 * 
 * Run this AFTER 04-fix-teamstats-tournament-structure.ts
 */
...
async function fixTeamStatsIdFormat() {
    const sql = neon(process.env.NEON_TOURNAMENT_DB_URL!);

    console.log('🚀 Starting TeamStats ID Format Fix (Add Season)');
    console.log('=============================================\n');
    ...
    try {
        ...
        // ========================================
        console.log('📊 Step 1/4: Analyzing current data...');
    ...
    `;
        ...
        // Check for duplicates (same team_id + tournament_id)
        const duplicates = await sql`
      SELECT team_id, tournament_id, COUNT(*) as count
      FROM teamstats
    ...
    `;
        ...
        if (duplicates.length > 0) {
            console.log(`  ⚠️  Found ${duplicates.length} duplicate team+tournament combinations`);
            console.log(`     This happens when multiple seasons share the same tournament_id (e.g., 'historical')\n`);
        }

        // ========================================
        // STEP 2: Create New Table with Correct Structure
        // ========================================
        console.log('📊 Step 2/4: Creating new table structure...');

        await sql`
      CREATE TABLE IF NOT EXISTS teamstats_new2 (
        id TEXT PRIMARY KEY,
        ...
        UNIQUE(team_id, season_id, tournament_id)
      )
    `;
        console.log('  ✅ Created teamstats_new2 table\n');

        // ========================================
        // STEP 3: Migrate Data with New ID Format
        // ========================================
        console.log('📊 Step 3/4: Migrating data to new structure...');
    ...
    `;
        ...
        for (const stat of allStats) {
            // NEW FORMAT: teamid_seasonid_tournamentid
            const newId = `${stat.team_id}_${stat.season_id}_${stat.tournament_id}`;

            try {
                await sql`
          INSERT INTO teamstats_new2 (
          ...
          ON CONFLICT (team_id, season_id, tournament_id) DO UPDATE SET
        ...
        `;
                ...
                console.log(`  ⚠️  Error migrating: ${stat.team_id} / ${stat.season_id} / ${stat.tournament_id}`);
        ...
        // STEP 4: Swap Tables
        // ========================================
        console.log('📊 Step 4/4: Swapping tables...');
        ...
        await sql`DROP TABLE IF EXISTS teamstats_old2`;
        console.log('  ✅ Dropped old backup table if existed');

        // Rename current table to backup
        await sql`ALTER TABLE teamstats RENAME TO teamstats_old2`;
        console.log('  ✅ Renamed current table to teamstats_old2');

        // Rename new table to teamstats
        await sql`ALTER TABLE teamstats_new2 RENAME TO teamstats`;
    ...
    `;
        ...
        await sql`CREATE INDEX IF NOT EXISTS idx_teamstats_team_season ON teamstats(team_id, season_id)`;
        console.log('  ✅ Created indexes\n');

        console.log('=============================================');
        console.log('✨ TeamStats ID Format Fix Complete!');
        console.log('=============================================\n');

        console.log('📋 Summary:');
        console.log(`   - ID format changed: teamid_tournamentid → teamid_seasonid_tournamentid`);
        console.log(`   - Primary key: id (unique)`);
        console.log(`   - Unique constraint: (team_id, season_id, tournament_id)`);
        console.log(`   - Old table preserved as: teamstats_old2\n`);
        ...
        console.log('   DROP TABLE teamstats_old2;\n');
