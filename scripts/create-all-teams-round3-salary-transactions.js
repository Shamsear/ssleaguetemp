...
async function processTeam(teamSeasonDoc, seasonId) {
  ...
  // Get fixture for round 3 from Neon
  ...
  if (fixtures.length === 0) {
    console.log(`   ⚠️  No fixture found for round 3 - SKIPPING`);
  ...
  const hasExisting = existingTxns.docs.some(doc => {
    ...
    return metadata.fixture_id === fixtureId && metadata.round_number === 3;
  });
  
  if (hasExisting) {
    console.log(`   ⚠️  Transactions already exist for round 3 - SKIPPING`);
...
async function createAllTeamsSalaryTransactions() {
  try {
    ...
    console.log('🎯 Processing ROUND 3 ONLY\n');
