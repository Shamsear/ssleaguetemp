/**
 * Create salary deduction transactions for 12 TEAMS (excluding Kopites and Portland Timbers)
 * Round 5 ONLY
 * 
...
async function processTeam(teamSeasonDoc, seasonId) {
  ...
  // Skip Kopites and Portland Timbers
  if (teamId === 'SSPSLT0023' || teamId === 'SSPSLT0026') {
    console.log(`\n${'='.repeat(60)}`);
    console.log(`⏭️  Skipping: ${teamName} (${teamId}) - Excluded from Round 5`);
  ...
  // Get fixture for round 5 from Neon
  ...
  if (fixtures.length === 0) {
    console.log(`   ⚠️  No fixture found for round 5 - SKIPPING`);
  ...
  const hasExisting = existingTxns.docs.some(doc => {
    ...
    return metadata.fixture_id === fixtureId && metadata.round_number === 5;
  });
  
  if (hasExisting) {
    console.log(`   ⚠️  Transactions already exist for round 5 - SKIPPING`);
...
async function createAllTeamsSalaryTransactions() {
  try {
    ...
    console.log('🎯 Processing ROUND 5 ONLY (excluding Kopites & Portland Timbers)\n');
    ...
    // Print summary
    console.log(`\n${'='.repeat(60)}`);
    console.log('📊 SUMMARY - ROUND 5 (12 teams, excluding Kopites & Portland Timbers)');
