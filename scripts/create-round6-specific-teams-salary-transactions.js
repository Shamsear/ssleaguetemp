/**
 * Create salary deduction transactions for SPECIFIC TEAMS
 * Round 6 ONLY - Portland Timbers, Legends FC, FC Barcelona, Red Hawks FC
 * 
...
async function processTeam(teamSeasonDoc, seasonId) {
  ...
  if (!ALLOWED_TEAM_IDS.includes(teamId)) {
    console.log(`\n${'='.repeat(60)}`);
    console.log(`⏭️  Skipping: ${teamName} (${teamId}) - Not in allowed list`);
    return { processed: false, reason: 'not_allowed' };
  }
  ...
  // Get fixture for round 6 from Neon
  ...
  if (fixtures.length === 0) {
    console.log(`   ⚠️  No fixture found for round 6 - SKIPPING`);
  ...
  const hasExisting = existingTxns.docs.some(doc => {
    ...
    return metadata.fixture_id === fixtureId && metadata.round_number === 6;
  });
  
  if (hasExisting) {
    console.log(`   ⚠️  Transactions already exist for round 6 - SKIPPING`);
...
async function createRound6SalaryTransactions() {
  try {
    const seasonId = 'SSPSLS16';
    
    console.log('🔍 Finding specific teams for season:', seasonId);
    console.log('🎯 Processing ROUND 6 ONLY');
    console.log('📋 Teams: Portland Timbers, Legends FC, FC Barcelona, Red Hawks FC\n');
    ...
    console.log(`\n${'='.repeat(60)}`);
    console.log('📊 SUMMARY - ROUND 6 (4 specific teams)');
...
createRound6SalaryTransactions();
