/**
 * Create salary deduction transactions for Manchester United
 * Rounds 1 and 2
 * 
...
async function createSalaryTransactions() {
  try {
    console.log('🔍 Finding Portland Timbers team...');
    
    // Find Portland Timbers team_season
    const teamSeasonsSnapshot = await db.collection('team_seasons')
      .where('team_name', '==', 'Portland Timbers')
      .where('status', '==', 'registered')
      .get();
    
    if (teamSeasonsSnapshot.empty) {
      console.error('❌ Portland Timbers team_season not found');
      return;
    }
    ...
    // Get fixtures for round 1 only from Neon
    console.log('\n🔍 Finding fixtures for round 1...');
    ...
    console.log(`   Found ${fixtures.length} fixtures for Portland Timbers in round 1`);
    
    if (fixtures.length === 0) {
      console.error('❌ No fixtures found for Portland Timbers in round 1');
      return;
    }
    ...
    for (const fixture of fixtures) {
      ...
      matchups.forEach(matchup => {
        // Determine if Manchester United was home or away
      ...
      console.log(`   Portland Timbers players: ${playerIds.size}`);
