...
try {
  ...
  files.forEach(file => {
    ...
    if (content.includes('update-stats') || content.includes('update-points')) {
      console.log(`File: ${path.relative(rootDir, file)}`);
      
      const lines = content.split('\n');
      lines.forEach((line, idx) => {
        if (line.includes('update-stats') || line.includes('update-points') || line.includes('calculate-points')) {
