...
async function processTeam(teamSeasonDoc, seasonId) {
  ...
  // Get fixture for round 1 from Neon
  ...
  if (fixtures.length === 0) {
    console.log(`   ⚠️  No fixture found for round 1 - SKIPPING`);
  ...
  const hasExisting = existingTxns.docs.some(doc => {
    ...
    return metadata.fixture_id === fixtureId && metadata.round_number === 1;
  });
  
  if (hasExisting) {
    console.log(`   ⚠️  Transactions already exist for round 1 - SKIPPING`);
