...
filesToFix.forEach(relPath => {
  ...
  // Clean up double opening parenthesis e.g. .map(((p: any) =>
  code = code.replace(/(\.map|\.filter|\.find|\.forEach|\.findIndex|\.some|\.every)\(\s*\(\s*\(\s*([a-zA-Z0-9_]+)\s*:\s*any\s*\)\s*=>/g, '$1(($2: any) =>');
...
console.log('Cleaned files count:', cleanedCount);
