...
// POST - Create or update round deadline (supports single object or array of objects)
export async function POST(request: NextRequest) {
  try {
    ...
    const items = Array.isArray(body)
      ? body
      : (Array.isArray(body.round_deadlines) ? body.round_deadlines : [body]);
    ...
    for (const item of items) {
      ...
      if (!tournament_id || !season_id || !round_number || !leg) continue;
    ...
    return NextResponse.json({
      success: true,
      message: `Successfully processed ${processedCount} round deadline(s)`
    });
  } catch (error: any) {
