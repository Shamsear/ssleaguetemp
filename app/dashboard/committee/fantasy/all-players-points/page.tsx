...
export default function CommitteeAllPlayersPointsPage() {
  ...
  const handleRecalculateAll = async () => {
    if (!confirm('Recalculate points for all players (including free agents), passive team bonuses, and fantasy standings?')) return;
    setIsRecalculating(true);
    try {
      const res = await fetchWithTokenRefresh('/api/admin/fantasy/recalculate-all-points', { method: 'POST' });
      if (res.ok) {
        alert('All fantasy points, free agent stats, team bonuses, and standings recalculated successfully!');
        loadPlayers();
      } else {
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="flex items-center gap-2 px-4 py-2.5 rounded-2xl bg-slate-900 hover:bg-slate-800 text-amber-400 font-mono text-xs font-extrabold uppercase tracking-wider transition-all shadow-sm disabled:opacity-50"
              >
