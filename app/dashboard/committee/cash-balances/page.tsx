...
export default function CommitteeCashBalances() {
  ...
  useEffect(() => {
    ...
    const loadSeasons = async () => {
      try {
        ...
        if (userSeasonId) {
          setSelectedSeasonId(userSeasonId);
        } else {
  ...
  if (loading || !user || (user.role !== 'committee_admin' && user.role !== 'super_admin')) {
    ...
    <div className="console-bg min-h-screen text-slate-800 relative pt-5 lg:pt-24 pb-8 sm:pb-12 px-4 sm:px-6 font-mono">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-xl sm:text-2xl font-black uppercase tracking-wider text-slate-800">
              Season Cash Balances <span className="text-amber-500">Overview</span>
            </h1>
            <p className="text-[10px] text-slate-500 font-bold uppercase mt-1">Committee Panel</p>
            {selectedSeasonId && (
              <p className="text-[10px] text-slate-550 uppercase font-bold mt-1">
                Season: <span className="text-amber-500 font-extrabold">{seasons.find(s => s.id === selectedSeasonId)?.name || selectedSeasonId}</span>
              </p>
            )}
          </div>
        </div>
                            ...
                            <div className="w-8 h-8 rounded-lg bg-slate-50 border border-slate-155 p-1 flex items-center justify-center relative overflow-hidden flex-shrink-0">
                             ...
                             <div className="flex flex-col">
                               <div className="flex items-center gap-1.5 flex-wrap">
                                 ...
                                 {(team as any).is_registered ? (
                                   <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[8px] font-black bg-green-50 text-green-700 border border-green-200/30 uppercase tracking-wider">Registered</span>
                                 ) : (
                                   <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[8px] font-black bg-slate-100 text-slate-450 border border-slate-200/30 uppercase tracking-wider">Not Registered</span>
                                 )}
                               </div>
                        ...
                        {selectedSeasonId !== 'all' && (
                          <td className="py-3.5 px-4 font-bold">
                            {user.role === 'super_admin' ? (
                              ...
                              <span
                                className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[9px] uppercase tracking-wider font-extrabold border ${
                                  currentPlan === 'upfront'
      ...
      {mounted && modalType === 'bulk_delete' && createPortal(
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-[9999] animate-fade-in font-mono text-slate-850">
          <div className="bg-white border border-rose-205 w-full max-w-md rounded-3xl shadow-2xl p-6 relative animate-slide-up space-y-4">
