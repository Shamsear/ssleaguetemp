  ...
  useEffect(() => {
    const loadLeagueData = async () => {
      ...
      try {
        ...
        if (!myTeamResponse.ok) throw new Error('Failed to fetch team');
        const myTeam = await myTeamResponse.json();
        
        setLeagueId(myTeam.team.league_id);

        // Get all rounds for this league
        const roundsResponse = await fetchWithTokenRefresh(`/api/fantasy/rounds?league_id=${myTeam.team.league_id}`);
    ...
    const loadPlayers = async () => {
      ...
      try {
        const url = selectedRound 
          ? `/api/fantasy/players/all-base-points?league_id=${leagueId}&round_id=${selectedRound}`
          : `/api/fantasy/players/all-base-points?league_id=${leagueId}`;
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-4 md:p-6">
      ...
      <div className="max-w-7xl mx-auto mb-6">
        <div className="flex items-center gap-3 mb-2">
          <SoccerBallIcon className="w-8 h-8 text-blue-600" />
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">
            All Players - Base Points
          </h1>
        </div>
        <p className="text-gray-600 dark:text-gray-400">
          View all players' base points (without captain/vice-captain multipliers) to plan your acquisitions
        </p>
      </div>
      ...
      <div className="max-w-7xl mx-auto mb-6 space-y-4">
        ...
        <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
          ...
          <select
            ...
            className="w-full md:w-64 p-2 border border-gray-300 dark:border-gray-600 rounded-lg 
