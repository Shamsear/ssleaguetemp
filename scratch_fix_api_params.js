...
filesToFix.forEach(relPath => {
  ...
  // Add explicit types to common single-param arrow functions
  code = code.replace(/(\.map|\.filter|\.find|\.forEach|\.findIndex|\.some|\.every)\(\s*([a-zA-Z0-9_]+)\s*=>/g, (m, fn, p) => `${fn}((${p}: any) =>`);
  // Clean up nested parens if created
  code = code.replace(/\(\(([a-zA-Z0-9_]+): any\)\s*=>/g, '($1: any) =>');
