...
async function debugPeriods() {
    ...
    // Check the playerStates for Skill 555 players
    const tid = 'SSPSLT0020';
    const players = ['sspslpsl0078', 'sspslpsl0020', 'sspslpsl0063', 'sspslpsl0002', 'sspslpsl0042'];

    // Re-run the rollback logic for these players to see where they are
    ...
    for (let p = 4; p > 0; p--) {
        ...
        for (const t of pTransfers) {
            if (t.player_in_id && playerStates.has(t.player_in_id)) {
                ...
                console.log(`P${p - 1} Rollback: ${t.player_in_id} in at window ${t.window_id}, setting P${p - 1} to null`);
            }
            if (t.player_out_id) {
                initPlayer(t.player_out_id);
                playerStates.get(t.player_out_id)[p - 1] = t.team_id;
                console.log(`P${p - 1} Rollback: ${t.player_out_id} out at window ${t.window_id}, setting P${p - 1} to ${t.team_id}`);
            }
        }
    }

    for (const pid of players) {
        console.log(`Player ${pid} States:`, playerStates.get(pid));
    }
}
